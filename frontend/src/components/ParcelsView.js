...
export default function ParcelsView({
  ...
  canDelete = true,
  title = "Warehouse",
  subtitle = "Manage incoming stock invoices and products.",
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || "Failed to load stock invoices");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon={Truck} label="Total invoices" value={stats?.total_parcels ?? "—"} accent="bg-neutral-900 text-white" />
        <StatCard icon={Package} label="Total packages" value={stats?.total_packages ?? "—"} accent="bg-blue-600 text-white" />
        <StatCard icon={Boxes} label="Total units" value={stats?.total_units ?? "—"} accent="bg-emerald-600 text-white" />
        <StatCard icon={CreditCard} label="Paid · Unpaid" value={`${stats?.paid ?? 0} · ${stats?.unpaid ?? 0}`} accent="bg-amber-600 text-white" />
      </div>
      ...
      <section className="bg-white border border-neutral-200 rounded-2xl">
        ...
        {loading ? (
          <div className="py-16 flex items-center justify-center text-neutral-400 text-sm gap-2">
            <Loader2 className="w-4 h-4 animate-spin" /> Loading stock invoices…
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-16 text-center px-6">
            ...
            <div className="mt-3 text-sm font-medium text-neutral-700">
              {parcels.length === 0 ? emptyTitle : "No matching invoices"}
            </div>
            <p className="text-xs text-neutral-500 mt-1">
              {parcels.length === 0 ? emptyHint : "Try a different search keyword."}
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="shrink-0">
                    {p.carton_photo ? (
                      ...
                      >
                        <img src={p.carton_photo} alt={p.parcel_number} className="w-full h-full object-cover" />
                      </button>
                    ) : (
                  ...
                  {/* main */}
                  <button onClick={() => setDetailParcel(p)} className="flex-1 min-w-0 text-left">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        ...
                        {p.products.length > 3 && (
                          <span className="text-neutral-400 font-normal"> +{p.products.length - 3} more</span>
                        )}
                      </span>
                    ...
                    <div className="mt-1 text-xs text-neutral-600 flex items-center gap-3 flex-wrap">
                      ...
                      <span className="inline-flex items-center gap-1 text-neutral-500">
                        <Boxes className="w-3.5 h-3.5" /> {p.total_quantity} units · {p.products.length}{" "}
                        {p.products.length === 1 ? "product" : "products"}
                      </span>
                      <span className="inline-flex items-center gap-1 text-neutral-500">
                    ...
                    <div className="mt-1.5 flex items-center gap-2 text-[11px] text-neutral-400">
                      ...
                      <span>·</span>
                      <span className="inline-flex items-center gap-1">
                        <Calendar className="w-3 h-3" /> {new Date(p.created_at).toLocaleString()}
                      </span>
                    </div>
                  </button>
      ...
      {canAdd && (
        <AddParcelModal open={addOpen} onClose={() => setAddOpen(false)} onCreated={handleCreated} />
      )}
